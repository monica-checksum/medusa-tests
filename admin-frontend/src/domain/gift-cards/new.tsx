  ...
  const onSubmit = async (data: NewGiftCardFormData) => {
    ...
    if (thumbnail) {
      const uploadedImgs = await Medusa.uploads
        ...
        .then(({ data }) => {
          // @ts-ignore
          window.checksum.goalTracker.recordGoalFinished("new-gift-card-thumbnail-upload-success")
    ...
    mutate(
      ...
      },
      {
        onSuccess: () => {
          // @ts-ignore
          window.checksum.goalTracker.recordGoalFinished("new-gift-card-success")
