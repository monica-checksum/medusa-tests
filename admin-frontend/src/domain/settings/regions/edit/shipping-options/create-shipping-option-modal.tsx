...
const CreateShippingOptionModal = ({ open, onClose, region }: Props) => {
  ...
  if (open) {
    if (!hasShownScreen) {
      // @ts-ignore
      window.checksum.goalTracker.recordGoalFinished("create-shipping-option-start")
  ...
  const onSubmit = handleSubmit((data) => {
    ...
    mutate(
      ...
      },
      {
        onSuccess: () => {
          // @ts-ignore
          window.checksum.goalTracker.recordGoalFinished("create-shipping-option-success")
