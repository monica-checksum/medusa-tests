  ...
  async ({
    ...
    await test.step("Wait for products table to load", async () => {
      ...
      await expect
        .poll(
          async () => {
            variablesStore.totalProductCount = await page
              .locator("tbody tr")
              .count()
    ...
    await test.step("Find existing cktest products", async () => {
      variablesStore.cktestProducts = page
        .locator("tbody tr")
        .filter({ hasText: new RegExp("cktest", "i") })
    ...
    await test.step("Create cktest product if none exists", async () => {
      if (variablesStore.cktestCount === 0) {
        await checksumAI(
          "Click New Product button to create a cktest product for deletion",
        ...
        await checksumAI(
          ...
          () =>
            page
              ...
              .fill(variablesStore.productToDelete)
        )
        ...
        await checksumAI(
          ...
          () =>
            page
              ...
              .fill(
                `${variablesStore.productToDelete} - Product for deletion testing`
              )
        )
        ...
        await checksumAI(
          ...
          () =>
            page
              ...
              .fill(
                `${variablesStore.productToDelete} - This product was created specifically for testing the delete functionality`
              )
        )
        ...
        variablesStore.firstProductText =
          variablesStore.cktestProductTexts.length > 0
            ? variablesStore.cktestProductTexts[0]
            : null

        if (variablesStore.firstProductText) {
          variablesStore.regexMatch = variablesStore.firstProductText.match(
            /cktest-\d+/
          )
          variablesStore.productToDelete = variablesStore.regexMatch
            ? variablesStore.regexMatch[0]
            : variablesStore.firstProductText.trim()
        } else {
          variablesStore.productToDelete = `cktest-${Date.now()}`
        }
      }
    ...
    await test.step("Locate product to delete using regex", async () => {
      variablesStore.productRow = page
        .locator("tbody tr")
        .filter({ hasText: new RegExp(variablesStore.productToDelete, "i") })
    ...
    await test.step("Verify product deletion using regex", async () => {
      ...
      variablesStore.cktestProductsAfterDeletion = page
        .locator("tbody tr")
        .filter({ hasText: new RegExp(variablesStore.productToDelete, "i") })
